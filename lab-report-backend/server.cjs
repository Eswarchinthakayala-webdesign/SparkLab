...
const app = express();
app.use(cors());
app.use(bodyParser.json({ limit: "30mb" })); // big limit for images/SVGs
...
app.post("/api/generate-report", async (req, res) => {
  try {
    // expected payload
    // { title, author, college, date, template, observations: [{V, I}], procedure, circuitImageBase64, chartSVG, calculationsHtmlOrText, conclusion }
    ...
    // --- Header (professional) ---
    // left: college logo (if provided), center: college name, right: metadata
   ...
   // author & date on the right
    ...
    // procedure may contain simple newline-separated steps
    ...
    // table: build simple table for observations (columns: V, I, etc)
    ...
    // --- Chart embedding: if svg provided, embed using svg-to-pdfkit at vector fidelity ---
    if (chartSVG) {
      try {
        doc.fontSize(12).fillColor("#ff9a4a").text("Auto-plotted Graph", { underline: true });
        doc.moveDown(0.4);

        // svg-to-pdfkit requires raw SVG. Place it at (x, y) with width
        const svgWidth = 500;
        const svgHeight = 300;
    ...
    if (circuitImageBase64) {
      ...
      if (img) {
        try {
          ...
          doc.moveDown(0.3);
          // save temp file? pdfkit can accept buffer via image method if image type is supported.
          // Add image with width constraint
          const imgOpts = { width: 420 };
    ...
    doc.moveDown(0.2);
    doc.fontSize(10).fillColor("#fff").text(payload.result || "Result placeholder");
